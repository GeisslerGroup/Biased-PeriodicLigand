...
#     plt.plot(bins_OG*180.0/np.pi, total_prob, color=c, label="centred at theta = %3.3f" %theta)
    plt.plot(bins_OG*180.0/np.pi, -np.log(total_prob), color=c, label="centred at theta = %3.1f" %(-theta*180.0/np.pi), linewidth=1)
...
eq_data = np.genfromtxt('/home/pratima/Biased-PeriodicLigand/dump_files/theta-0.93-new.txt', delimiter=' ')
...
#     print [ (itr_where % 240, itr_where/240) for itr_where in np.where(data < -1.0)[0] ]
    ...
    theta = -0.93
    print theta, np.mean(data), np.std(data)
    plt.plot(bins_OG, total_prob, color=c, label="centred at theta = %3.3f" %theta)
