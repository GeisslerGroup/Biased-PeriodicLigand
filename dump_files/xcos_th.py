...
data = np.genfromtxt('/home/pratima/Biased-PeriodicLigand/dump_files/lig.-0.50', delimiter=' ')
...
for i in range (start,end,18):
  if (i + 17 < size):
    ...
    x_vec = (x_vec - round(x_vec)) * (xb1 - xb0)
    thx = np.arctan(x_vec / y_vec)
    f.write("%4.5f\n" %(thx))

f.close()

hist_data = np.genfromtxt('/home/pratima/Biased-PeriodicLigand/dump_files/theta_x-0.50.txt', delimiter=' ')
...
bins = np.linspace(-1.70, 1.70, 200)
